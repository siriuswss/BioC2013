...
# SE <- read.table(file.path("SE.bed"), header=FALSE)
# names(SE) <- c("chromosome", "start", "end", "name", "score", "strand")
# SE <- as(SE, "GRanges")
# Read .bed format using rtracklayer package
library(rtracklayer)
SE<-import(file.path("SE.bed"))
...
seg<-segmentPING(data=SE, minReads=NULL, maxLregion=1200, minLregion=80, jitter=TRUE)
summary(seg)
@
...
\subsection{Post-processing PING results}
...
\end{Exercise}
<<Answer,eval=FALSE,echo=FALSE,fig=FALSE>>=
write.table(PS, file="postPING.txt", sep="\t", row.names=FALSE)
@
...
plotSummary(PS, ping, SE, "chrIII", "gen", from=152000, to=156000)
@

The plot shows the predicted nucleosome positions, the associated score for each nucleosome and the profile of the short reads in addition to the alighned reads. The user can filter the nucleosomes passing some threshold of the score using the argument of \Rclass{scoreThreshold}. 
 
\begin{Exercise}
Replot the predicted nucleosome positions for the same genomic region with different threshold for score track.
\end{Exercise}

<<Answer,eval=FALSE,echo=FALSE,fig=FALSE>>=
plotSummary(PS, ping, SE, "chrIII", "gen", from=152000, to=156000, scoreThreshold=10)
@
...
<<Visualization, eval=FALSE, echo=TRUE>>=
library(Gviz)
cTrack <- CoverageTrack(ping, SE, "chrIII", "gen")
rTrack <- RawReadsTrack(ping, SE, "chrIII", "gen", name = "Reads")
nTrack <- NucleosomeTrack(PS, "chrIII", "gen", scoreThreshold = 0.1, name = "NEW")

## Add genomic information using Gviz's built-in functions. 
gTrack <- GenomeAxisTrack(add53 = TRUE, add35 = TRUE)
aTrack <- AnnotationTrack(start=154000, end=155000, showFeatureId= TRUE,
     id="random annotation", col.title = "orange", chr = "chrIII",
     gen = "gen", name = "custom")
plotTracks(trackList = c(gTrack, cTrack, aTrack, rTrack, nTrack),
     main = "Custom plot", from = 152000, to = 156000)
@ 
